...
def cluster_chains_statistics(group):
    """ 统计每一类的出行链分布
    """
    def sequence_to_chain(ls):
        """ encode list to tuple of [char,weight]
        ls = list('HHHTTHHHWWWTTTWWWTTTTHHHH')
        'HHHTTHHHWWWTTTWWWTTTTHHHH' --> [['H', 3], ['T', 2], ['H', 3], ['W', 3], ['T', 3], ['W', 3], ['T', 4], ['H', 4]]
        """
        c0 = ls[0]
        w = 1
        i = 1
        ...
        r.append([c0,w])
        chain = '-'.join([x[0] for x in r])
        return chain

    chains = dict()
    for i in range(len(group)):
        person = group.index[i]
        chain = sequence_to_chain(group.iloc[i].values)
        chains[person] = chain

    chains = pd.DataFrame.from_dict(chains,orient='index')
    chains.columns = ['chain']
    group_cnt = chains.groupby('chain').chain.count().sort_values(ascending=False) / len(group)
    return group_cnt
